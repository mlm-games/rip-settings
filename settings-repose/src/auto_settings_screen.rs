...
pub fn AutoSettingsScreen<T: SettingsSchema>(
    ...
    for (category, fields) in groups {
        let display_title = title_map.get(&category).cloned().unwrap_or_else(|| category.clone());
        let mut field_views: Vec<View> = Vec::new();
        for field in fields {
            let enabled = value.is_field_enabled(&field);
            let title = field.title;
            let desc = if field.description.is_empty() { None } else { Some(field.description) };
            ...
            let fv: View = match &field.kind {
                FieldKind::Toggle => {
                    let raw = value.get_field_value(field.name).unwrap_or(serde_json::Value::Bool(false));
                    ...
                    SettingsToggle(
                        ...
                        move |new_val| {
                            ...
                            if let Some(rules) = validation_rules.clone() {
                                if let validation::ValidationResult::Invalid(msg) = validation::validate_value(&json, &rules) {
                ...
                FieldKind::Slider { min, max, step } => {
                    let raw = value.get_field_value(field.name).unwrap_or(serde_json::json!(min));
                ...
                FieldKind::Button { action } => {
                    ...
                    SettingsAction(title, desc, enabled, move || { log::info!("action: {act}"); }, None)
                }
                FieldKind::TextInput => {
                    let raw = value.get_field_value(field.name).unwrap_or(serde_json::Value::String(String::new()));
                    let cur = raw.as_str().unwrap_or("").to_string();
                    let subtitle = if cur.is_empty() { "(empty)".to_string() } else { cur.clone() };
                    let name = field.name.to_string();
                    let on_set_c = on_set.clone();
                    let ov = overlay.clone();
                    let f_title = field.title.to_string();
                        ...
                        move || {
                            ...
                            let _dlg = InputDialog(
                                ...
                                Rc::new({
                                    let on_set_c = on_set_c.clone();
                                    let name = name.clone();
                                    move |new_val: String| on_set_c(&name, serde_json::Value::String(new_val))
                                }),
                            );
                            state.show();
                        },
        ...
        for v in field_views { inner = inner.child(v); }
    ...
    for v in category_views { outer = outer.child(v); }
    outer
}
